...
void MainWindow::mousePressEvent(QMouseEvent* event)
{
    ...
    {
        counter = 0; // номер точки может быть равен 0 или 1
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    int lv, ln, pv, pn, min, fl;
    int x = p[0].x();
    int y = p[0].y();



    if (counter == 1) // если окружность задана, рисуем ее
    {
        lv = sqrt(x * x + y * y);
        ln = sqrt(x * x + (height() - y) * (height() - y));
        pv = sqrt((width() - x) * (width() - x) + y * y);
        pn = sqrt((width() - x) * (width() - x) + (height() - y) * (height() - y));
        ...
        painter.setPen(QPen(QColor(0,255,0), 2)); // перо

        switch (fl)
        {
        case 0:
        {
            painter.drawLine(x,y,0,0);
            painter.drawLine(x,y,x,y+min);
            break;
        }
        case 1:
            painter.drawLine(x,y,0,height()); break;
        case 2:
            painter.drawLine(x,y,width(),0); break;
        case 3:
            painter.drawLine(x,y,width(),height()); break;
        }
